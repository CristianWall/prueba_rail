...
def generate_frames():
    ...
    while True:
        if not OPENCV_AVAILABLE:
            # Si OpenCV no está disponible, enviar frame de placeholder
            frame_bytes = create_placeholder_frame("OpenCV no disponible en este entorno")
        else:
            cam = get_camera()
            if cam is None or not cam.isOpened():
                frame_bytes = create_placeholder_frame("Cámara no disponible")
            else:
                try:
                    success, frame = cam.read()
                    if not success:
                        frame_bytes = create_placeholder_frame("Error al leer cámara")
                    else:
                        # Codificar el frame como JPEG
                        ret, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
                        if not ret:
                            frame_bytes = create_placeholder_frame("Error al codificar frame")
                        else:
                            frame_bytes = buffer.tobytes()
                except Exception as e:
                    print(f"Error en generate_frames: {e}")
                    frame_bytes = create_placeholder_frame(f"Error: {str(e)}")
